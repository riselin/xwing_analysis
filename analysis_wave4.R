...
d.database <- read.csv("./database5.csv", header = T, sep = ";")
...
d.database[,12] <- as.character(d.database[,12]) #size
...
readFromFolder <- function(){
  d.wide <- data.frame()
  temp <-  list.files(path = "./wave4/parsed/", pattern="*.csv")
  for (i in 1:length(temp)){
    temppath <- paste0("./wave4/parsed/", temp[i])
    d.temp <- read.csv(temppath, header = T, sep = ",")
    lists_possible <- c(lists_possible, max(d.temp$listID))
    d.temp <- tournamentAssembly(d.temp, ffrounds = 6)
...
winrates_cut <- ggplot(winpilots_cut, aes(x=total_games, y=percentage)) +
  ...
  coord_flip() +
  geom_text(aes(label=pilot,hjust=0.5, vjust=-0.5, color = faction), size = 3.5)+
  scale_color_manual(values = c("red2","darkgreen",  "springgreen3", "sienna2",  "purple3", "dodgerblue4"))
...
rm(ls = fiveawings, fiveawings_ids, d.rz2, d.5rz2)
#do with getPerSquad
